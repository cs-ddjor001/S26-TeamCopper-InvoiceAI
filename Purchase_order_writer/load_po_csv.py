...
from models.po_line_item import PO_Line_Item
...
def normalize_vendor(name):
    """Collapse multiple internal spaces and strip surrounding whitespace."""
...
def load_purchase_orders(filepaths=None):
    ...
    # Preload existing PO numbers so we don't insert duplicate headers
    ...
    for filepath in filepaths:
        ...
        with open(filepath, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                # Insert PO header once per unique PO_NUMBER
                if po_number not in po_cache:
                    ...
                    db.session.add(po)
                    db.session.flush()  # populate po.id before referencing it
